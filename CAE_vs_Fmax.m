...
%    数据来源：Python仿真保存的 cae_vs_fmax.mat 文件
...
% 1. 读取Python的.mat数据
% --------------------------
mat_file = 'result/cae_vs_fmax.mat';
data = load(mat_file);

Fmax = data.Fmax;          % X轴：0.1~1.0（10个点）

% CAE数据
lp_cae = data.lp_cae;      % LP算法CAE
rvi_cae = data.rvi_cae;    % RVI算法CAE
fps_cae = data.fps_cae;    % FPS算法CAE
dpp_cae = data.dpp_cae;    % DPP算法CAE
...
% 绘制CAE曲线
plot(Fmax, lp_cae,     'b-o',  'MarkerSize',7, 'LineWidth',1.2, 'DisplayName','LP');
plot(Fmax, rvi_cae,    'r-s',  'MarkerSize',7, 'LineWidth',1.2, 'DisplayName','RVI');
plot(Fmax, fps_cae,    'g-^',  'MarkerSize',7, 'LineWidth',1.2, 'DisplayName','FPS');
plot(Fmax, dpp_cae,    'm-d', 'MarkerSize',7, 'LineWidth',1.5, 'DisplayName','DPP');
...
all_cae_valid = all_cae(~isnan(all_cae));
ylim([min(all_cae_valid)-0.5, max(all_cae_valid)+0.5]);

% 导出
export_ieee_pdf(fig1,'figures/CAE_Fmax',figW,figH,paperW,paperH);
...
xlabel(['$F_{\max}$'],...
    'Interpreter','latex','FontSize',11,'FontName','Times New Roman')
ylabel('$F_1$','Interpreter','latex','FontSize',11,'FontWeight','bold','FontName','Times New Roman');
...
xlabel(['$F_{\max}$'],...
    'Interpreter','latex','FontSize',11,'FontName','Times New Roman')
ylabel('$F_2$','Interpreter','latex','FontSize',11,'FontWeight','bold','FontName','Times New Roman');
...
% 导出
export_ieee_pdf(fig2,'figures/CAE_F1F2Fmax',figW,figH,paperW,paperH);
...
% 导出
export_ieee_pdf(fig3,'figures/CAE_SingleBothFmax',figW,figH,paperW,paperH);

% --------------------------
% 6. 输出完成提示 + 数据验证
% --------------------------
fprintf('从按算法拆分的.mat文件读取数据并生成三张IEEE标准图完成：\n');
fprintf('- Fig1_CAE_Performance.pdf (CAE性能对比)\n');
