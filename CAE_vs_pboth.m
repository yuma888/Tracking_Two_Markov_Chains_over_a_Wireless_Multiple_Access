...
%    数据来源：Python仿真保存的 cae_vs_p_both.mat 文件
% ============================================================
...
% 1. 读取Python的.mat数据
% --------------------------
mat_file = 'result/cae_vs_p_both.mat';
data = load(mat_file);

p_both = data.p_both;    % X轴

% === CAE ===
lp_cae  = data.lp_avg_cae;
rvi_cae = data.rvi_avg_cae;
fps_cae = data.fps_avg_cae;
dpp_cae = data.dpp_avg_cae;
...
plot(p_both, lp_cae,  'b-o',  'MarkerSize',7, 'LineWidth',1.2, 'DisplayName','LP');
plot(p_both, rvi_cae, 'r-s',  'MarkerSize',7, 'LineWidth',1.2, 'DisplayName','RVI');
plot(p_both, fps_cae, 'g-^',  'MarkerSize',7, 'LineWidth',1.2, 'DisplayName','FPS');
plot(p_both, dpp_cae, 'm-d', 'MarkerSize',7, 'LineWidth',1.5, 'DisplayName','DPP');
...
all_cae = [lp_cae; rvi_cae; fps_cae; dpp_cae];
valid = all_cae(~isnan(all_cae));
ylim([min(valid)-0.5, max(valid)+0.5]);

export_ieee_pdf(fig1,'figures/CAE_vs_pboth',figW,figH,paperW,paperH);
...
legend('Location','best'); xlim([min(p_both) max(p_both)]); ylim([0.3 0.7]);
...
legend('Location','best'); xlim([min(p_both) max(p_both)]); ylim([0.4 0.7]);

export_ieee_pdf(fig2,'figures/CAE_F1F2_vs_pboth',figW,figH,paperW,paperH);
...
legend('Location','best'); ylim([0 1.1]); xlim([min(p_both) max(p_both)]);
...
legend('Location','best'); ylim([0 0.4]); xlim([min(p_both) max(p_both)]);

export_ieee_pdf(fig3,'figures/CAE_singleSum_and_both_vs_pboth',figW,figH,paperW,paperH);


fprintf("\n 成功从 cae_vs_p_both.mat 读取数据，并绘制三张 IEEE 图！\n");
