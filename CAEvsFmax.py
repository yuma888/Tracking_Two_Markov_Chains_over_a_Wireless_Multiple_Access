...
# ============================================================
# Monte-Carlo Simulation (Independent of CMDP's internal simulate_transmission)
# ============================================================
def simulate_policy_mc(cmdp, policy, num_steps=50000, burn_in=2000, count_actions=True):
    """
    Perform Monte-Carlo simulation to evaluate a given stationary randomized policy.
    Compatible with TwoSourceMPRCMDP without relying on its internal simulate_transmission method.
    ...
    Parameters:
        ...
        burn_in (int): Burn-in steps to reach steady state (not included in statistics).
    ...
    for t in range(num_steps + burn_in):
        ...
        # Calculate one-step CAE for the current state-action pair
        cae = cmdp.c_sa[state, a_idx]
...
# ============================================================
# Main Evaluation Function: CAE vs Fmax
# ============================================================
def evaluate_cae_vs_fmax(p_fixed=0.5, V_dpp=100, steps=20000, burn_in=2000):
    """
    ...
    print(f"\n====== Running: CAE vs Fmax (p={p_fixed}) ======\n")

    # Iterate over each Fmax value
    for fmax in f_list:
        print(f">>> Fmax = {fmax:.1f}")
        # Initialize the two-source MPR-CMDP model with current Fmax constraint
        ...
        try:
            ...
            row["LP"] = mc_lp
            print(f"  LP   CAE={mc_lp['avg_cae']:.4f}")
        ...
        try:
            ...
            row["RVI"] = mc_rvi
            print(f"  RVI  CAE={mc_rvi['avg_cae']:.4f}")
        except Exception as e:
            print("  RVI FAILED:", e)
        ...
        try:
            ...
            row["FPS"] = mc_fix
            print(f"  FPS  CAE={mc_fix['avg_cae']:.4f}")
        except Exception as e:
            print("  FPS FAILED:", e)
...
# ============================================================
# New: Save Results Split by Algorithm
# ============================================================
def save_results_for_matlab(results, mat_filename="cae_vs_fmax.mat", csv_filename="cae_vs_fmax.csv"):
    """
    ...
    Example: lp_cae = [value at Fmax=0.1, value at Fmax=0.2, ..., value at Fmax=1.0]

    Parameters:
        results (list): Output of evaluate_cae_vs_fmax, containing results for each Fmax value.
        mat_filename (str): Path to save the MAT file (default: "cae_vs_fmax.mat").
        csv_filename (str): Path to save the backup CSV file (default: "cae_vs_fmax.csv").
    """
    ...
    for f_idx, row in enumerate(results):
        for alg in alg_list:
            ...
            mat_data[f"{alg.lower()}_cae"][f_idx] = result_alg["avg_cae"]
...
def print_results_table(results):
    """
    Print a formatted table of simulation results, including CAE, transmission frequencies,
    ...
    print("=" * 150)
    # Table header
    print(f"{'Fmax':<6}{'Alg':<6}{'CAE':<10}{'F1':<8}{'F2':<8}"
    ...
    for row in results:
        ...
        for alg in ["LP", "RVI", "FPS", "DPP"]:
            ...
            if result_alg is None:
                ...
                continue
            # Print formatted metrics (4 decimal places for CAE, 3 for others)
            print(f"{F:<6.1f}{alg:<6}"
                  f"{result_alg['avg_cae']:<10.4f}"
...
if __name__ == "__main__":
    # Run the main evaluation (CAE vs Fmax)
    results = evaluate_cae_vs_fmax(
    ...
    save_results_for_matlab(results, "result/cae_vs_fmax.mat")